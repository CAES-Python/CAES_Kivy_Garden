...
primitive datetime selector::
    ...
    if __name__ == '__main__':
        ...
        from kivy.garden.vertical_gauge import Vertical_Gauge, CyclicVertical_Gauge, \
            TimeFormatCyclicVertical_Gauge
...
from kivy.garden.gauge_tickline import Tick, Tickline, TickLabeller
...
class Vertical_Gauge(Tickline):
    ...
    '''The class of the tick in this vertical_gauge. Defaults to
    ...
    rolling_value = AliasProperty(get_rolling_value,
                                    set_rolling_value,
                                    bind=['index_mid'])
    '''the val indicated by whatever slot is in the middle of the vertical_gauge.
    If the vertical_gauge is still, then :attr:`rolling_value` is equal to
    ...
    def __init__(self, **kw):
        ...
        super(Vertical_Gauge, self).__init__(**kw)
        self.scale = dp(10)
    ...
    def _update_effect_constants(self, *args):
        if not super(Vertical_Gauge, self)._update_effect_constants(*args):
            return
        effect = self.scroll_effect
        scale = self.scale
        effect.pull_back_velocity = sp(50) / scale
    def calibrate_scroll_effect(self, *args, **kw):
        if not super(Vertical_Gauge, self).calibrate_scroll_effect(*args, **kw):
            return
...
class CyclicVertical_Gauge(Vertical_Gauge):
    '''vertical_gauge for displaying cyclic values.'''
    ...
    def __init__(self, **kw):
        super(CyclicVertical_Gauge, self).__init__(**kw)
    ...
    def on_tick(self, *args):
        ...
        super(CyclicVertical_Gauge, self).on_tick(*args)
...
class TimeFormatCyclicVertical_Gauge(CyclicVertical_Gauge):
    '''formatted vertical_gauge for displaying time.'''
